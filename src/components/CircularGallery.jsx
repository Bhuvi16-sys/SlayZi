...
class App {
  ...
  onTouchUp(e) {
    ...
    // Detect click if drag distance is tiny
    if (!e) return;
    const clientX = e.changedTouches ? e.changedTouches[0].clientX : e.clientX;
    if (this.start !== undefined && clientX !== undefined) {
      const dragDistance = Math.abs(this.start - clientX);
      if (dragDistance < 8 && this.onItemClick) {
        const rect = this.container.getBoundingClientRect();
        const clickX = clientX - rect.left;
        
        // Calculate viewport coordinate
        const viewportX = (clickX / rect.width) * this.viewport.width - this.viewport.width / 2;
        ...
        if (clickedMedia && minDist < clickedMedia.plane.scale.x / 2 + 0.5) {
          this.onItemClick(clickedMedia.index % originalLength);
        }
      }
    }
  }
